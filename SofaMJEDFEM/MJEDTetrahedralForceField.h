...
public:
	...
	void setalpha(const vector<Real> alpha) {
		f_parameterAlpha.setValue(alpha);
	}
	void settau(const vector<Real> tau) {
		f_parameterTau.setValue(tau);
	}
	void setviscous (const bool viscous){
		viscoelasticity.setValue(viscous);
	}
